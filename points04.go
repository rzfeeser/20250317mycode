...
func (u *User) newPet(pettoadd string) {
 u.Pets = append(u.Pets, pettoadd)        // Simple function should ensure "Lucy" is added to User
 fmt.Println(*u)                         // This user is **NOT** the same user as the one in main()!!
}

func main() {
 u := User{Name: "Anna", Pets: []string{"Bailey"}}
 u.newPet("Lucy")                                         // {Anna [Bailey Lucy]} -- This *should* add "Lucy" to "u"
